...
// open forms
	...
	//animation	
		...
		//$('.s_bg').css({'opacity':'0', 'transform':'translate3d(0px,0px, 0px'}).animate({opacity: '1'},0);
	};teleScreen()
...
// menu burger
		...
		$('.xmenu').click(function(){
				if(xmenuToggle === true) {
					...
					xmenuToggle = false;
					$('.deshevle').hide();
                    $('#mens').addClass('mobile_menu_blok'); 
				}
				else {
					$('header').removeClass('mobile');
					xmenuToggle = true;
					$('.deshevle').show();
                    $('#mens').removeClass('mobile_menu_blok');
				}
		});
	...
	$(".send_mail").click(function() {
		...
		data: dataString,
			success: function() {
				...
				smoke.alert("СПАСИБО ЗА ЗАЯВКУ!<br>МЫ СВЯЖЕМСЯ С ВАМИ В БЛИЖАЙШЕЕ ВРЕМЯ."); 
			}
		});
		return false;
	});
